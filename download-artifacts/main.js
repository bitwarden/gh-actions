...
async function downloadAction(name, path) {
  // @actions/artifact v6: use default export instead of create()
  const artifactClient = artifact.default;

  // @actions/artifact v6: downloadArtifact() now requires artifact ID
  // First, get the artifact metadata to retrieve its ID
  const artifactInfo = await artifactClient.getArtifact(name);

  // Then download using the artifact ID
  const downloadResponse = await artifactClient.downloadArtifact(
    artifactInfo.artifact.id,
    {
      path: path,
      // Note: createArtifactFolder option is not supported in v6
    },
  );
  ...
  try {
    ...
    uniqueInputSets.forEach((inputSet) => {
      ...
      if (providedInputs.length > 1) {
        throw new Error(
          `The following inputs cannot be used together: ${Object.keys(inputSet).join(', ')}`,
        );
      }
    });
    ...
    const matchesWithRegex = (stringToTest, regexRule) => {
      const escapeSpecialChars = (string) =>
        string.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, '\\$1');
      const builtRegexRule =
        '^' + regexRule.split('*').map(escapeSpecialChars).join('.*') + '$';
    ...
    artifacts = artifacts.filter((artifact) => {
      return artifactNames
        .map((name) => matchesWithRegex(artifact.name, name))
