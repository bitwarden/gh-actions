...
def get_github_api_response(url, action_id):
    ...
    retry = Retry(3, backoff_factor=0.1, raise_on_status=True,
...
def get_action_update(action_id):
    ...
    if "bitwarden" in path:
        ...
        r = get_github_api_response(f"https://api.github.com/repos/{path}/releases/latest", action_id)
        if r is None:
            return None
        ...
        r = get_github_api_response(f"https://api.github.com/repos/{path}/git/ref/tags/{tag_name}", action_id)
        if r is None:
            return None
